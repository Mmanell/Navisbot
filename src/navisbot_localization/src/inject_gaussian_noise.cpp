#include <navisbot_localization/inject_gaussian_noise.hpp>
...
InjectGaussianNoise::InjectGaussianNoise(const std::string& name)
                                  : Node(name)
                                  ...
                                  , theta_(0.0)
{
    ...
    joint_sub_ = create_subscription<sensor_msgs::msg::JointState>("/joint_states", 10, std::bind(&InjectGaussianNoise::jointCallback, this, _1));
    noisy_joint_pub_ = create_publisher<nav_msgs::msg::Odometry>("/navisbot_controller/odom_noisy", 10);
...
void InjectGaussianNoise::jointCallback(const sensor_msgs::msg::JointState &state)
{
    ...
    noisy_joint_pub_->publish(odom_msg_);
...
int main(int argc, char* argv[])
{
  ...
  auto node = std::make_shared<InjectGaussianNoise>("inject_gaussian_noise");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}